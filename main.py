...
from config import START_FROM, START_URL, MAX_DOWNLOAD_WORKERS, MAX_PROCESS_WORKERS, DEST_FOLDER
...
async def main():
    async with async_timer("Полный цикл загрузки данных SPIMEX"):
        ...
        try:
            async with ParserLinkAsync(START_FROM, START_URL) as parser, \
                ...
                producer_task = asyncio.create_task(
                    link_producer(parser, url_queue, MAX_DOWNLOAD_WORKERS)
                )
                
                # Запускаем скачиваем файлы и кладём в очередь
                download_tasks = [
                    asyncio.create_task(
                        download_worker(url_queue, file_queue, downloader, DEST_FOLDER)
                    )
                ...
                processor_tasks = [
                    asyncio.create_task(
                        file_processor_worker(file_queue, record_queue, executor, START_FROM)
                    )
                ...
                db_task = asyncio.create_task(
                    db_writer_worker(record_queue, db_writer)
                )
